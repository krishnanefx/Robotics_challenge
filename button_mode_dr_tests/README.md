...
button-mode sketch. Each sketch is interactive over the serial monitor
...
| `dead_reckoning_test` | Encoder + gyro fusion | The grid path that produced the tuned dead-reckoning values; RFID early-stop trims drift at tagged nodes. |
| `arena_rfid_route_test` | Full dead-reckoning route | Same dead-reckoning controller with LED calibration status + auto-run, RFID as an early node stop. |
